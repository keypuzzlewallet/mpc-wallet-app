...
enum EIP1193 {
  ...
  /// Pass when web app switch chain
  switchEthereumChain,
}
...
class DefiBrowser extends StatefulWidget {
  const DefiBrowser({
    Key? key,
  ...
  /// If use [DefiBrowser]'s provider provide is [PosiProvider]. Please provide [rpcUrl], [chainId], [walletAddress].
  ...
  @override
  State<DefiBrowser> createState() => _DefiBrowserState();
}

class _DefiBrowserState extends State<DefiBrowser> {
  ...
  Future<void> _loadWeb3() async {
    ...
    String path = widget.customPathProvider ??
        'assets/defibrowser/posi.min.js';
  ...
  Future<void> _jsBridgeCallBack(String message) async {
    print("_jsBridgeCallBack: $message");
    Map<dynamic, dynamic> rawData = JsonUtil.getObj(message);
    final name = rawData["name"];
    if (name == 'switchEthereumChain' || name == 'wallet_switchEthereumChain') {
      widget.signCallback(rawData, EIP1193.switchEthereumChain, _webViewController);
