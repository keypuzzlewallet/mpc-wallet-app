...
class CoinManagementController extends FxController {
  String searchKeyword = "";
  List<BlockchainCoinItem> items = [];

  CoinManagementController();
  ...
  @override
  String getTag() {
    return "coin_management_controller";
  }

  coinSelected(WalletEntity wallet, BlockchainCoinItem item, bool enabled) {
    if (enabled) {
      List<EnabledBlockchain> blockchains = wallet.enabledBlockchains.where((element) => element.blockchain == item.blockchainConfig.blockchain).toList();
      if (blockchains.isEmpty) {
        wallet.enabledBlockchains.add(EnabledBlockchain(blockchain: item.blockchainConfig.blockchain, coins: [item.coinConfig.coin]));
      } else {
        blockchains[0].coins.add(item.coinConfig.coin);
      }
      getIt<WalletService>().updateWalletEnabledBlockchains(wallet.walletId, wallet.enabledBlockchains);
    } else {
      List<EnabledBlockchain> blockchains = wallet.enabledBlockchains.where((element) => element.blockchain == item.blockchainConfig.blockchain).toList();
  ...
  List<BlockchainCoinItem> search(List<BlockchainCoinItem> items) {
    ...
    return items
        .where((element) =>
            element.toString().contains(searchKeyword.toLowerCase()))
        .toList();
  }

  bool getSwitchValue(WalletEntity wallet, BlockchainCoinItem item) {
    var enabledBlockchain = wallet.enabledBlockchains
        .where((element) => element.blockchain == item.blockchainConfig.blockchain)
        .toList();
